...
def eye_aspect_ratio(eye):
    ...
    # Computing the eye landmark's horizontal distance
    C = dist(eye[0], eye[3])
...
while True:
    ...
    # Detecting faces using both detectors
    faces_dlib = detector(gray)
    faces_opencv = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))

    # Check if either face detection method found a face
    if len(faces_dlib) == 0 or len(faces_opencv) == 0:
        lock_pc()
        continue  # Skip processing if a face is not found
