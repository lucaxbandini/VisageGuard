...
BLINK_TIMEOUT_DEFAULT = 2  # Default blink timeout in seconds

# Initialize counters, booleans, and variables for blinking detection
...
def start_detection():
    ...
    while detection_active:
        ...
        if ret:
            ...
            # Check if blinking isn't detected for more than the specified timeout
            if not BLINKING and time.time() - last_blink_time > blink_timeout.get():
                lock_pc()

            # Display the resulting frame in the GUI
...
blink_timeout.set(BLINK_TIMEOUT_DEFAULT)  # Default value
